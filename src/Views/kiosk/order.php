<section class="kiosk-order">
  ...
  <div class="order-layout">
    ...
    <div class="menu-panel">
      ...
      <div class="menu-tabs">
        <?php $first = true; foreach ($menu as $group) : ?>
        <button type="button"
      ...
      <div class="menu-items">
        ...
        <div class="menu-category" data-cat="<?= $group['category']->id ?>">
          ...
          <button type="button" class="menu-item" onclick="addToCart(<?= $item->id ?>, '<?= htmlspecialchars($item->name, ENT_QUOTES) ?>', <?= $item->price ?>)">
    ...
    <div class="cart-panel">
      ...
      <form method="POST" action="/kiosk/order" id="order-form">
        ...
        <div class="order-options">
          <label class="option">
            <input type="radio" name="order_type_radio" value="DINE_IN" checked onchange="setType(this.value)"> Dine In
          </label>
          <label class="option">
            <input type="radio" name="order_type_radio" value="TAKEAWAY" onchange="setType(this.value)"> Takeaway
          </label>
          <label class="option">
            <input type="radio" name="order_type_radio" value="DELIVERY" onchange="setType(this.value)"> Delivery
          </label>
        </div>
        ...
        <div class="form-group" id="table-select-group">
          ...
          <select id="table-select" onchange="document.getElementById('order-table').value = this.value">
            ...
            <option value="<?= $table->id ?>">Table <?= $table->number ?> (<?= $table->capacity ?>)</option>
            <?php endforeach ?>
          </select>
        </div>

        <button type="submit" class="button button-primary place-order-btn" onclick="return submitOrder()">Place Order</button>
      </form>
    </div>
